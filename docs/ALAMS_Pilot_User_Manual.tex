...
Edit \texttt{.env.local} and map the server LAN endpoint (e.g., if server runs at IP \texttt{10.0.3.5}):
...
\end{lstlisting}
\end{tcolorbox}
The console will start running on port 3000. You can access it at \texttt{http://10.0.3.5:3000}.
...
\begin{enumerate}
  \item Navigate to the admin web console: \texttt{http://10.0.3.5:3000}.
